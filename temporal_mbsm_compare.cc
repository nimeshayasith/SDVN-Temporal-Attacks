...
static const double   MAX_ACCEL_MS2  = 2.6;   // paper Table 2: maximum vehicle acceleration (m/s²)
...
static void TEMP_RSUSocketReceive(Ptr<Socket> sock)
{
    ...
    while ((pkt = sock->Recv()))
    {
        ...
        // oracle ensures that MBSM_Detect's failure to fire on those BSMs is counted
...
static void TEMP_TTW_S2_RSUReplayAttack(uint32_t v1_id, uint32_t v0_id, double forged_time)
{
    ...
    // Only the stale BSM injected to the controller (rsu_forward below) is the attack event.
...
{
    // Counter selection depends on which layer the detection actually happens:
    //
    //   S2, S4, S6, S8 (malicious RSU or controller WITH RSU):
    //     Detection occurs at CONTROLLER level — controller compares vehicle-direct BSM
    ...
    //     MBSM_Detect runs at RSU level only. No-RSU scenarios → RSU never processes BSMs.
    //     Controller cross-check for S1/S3/S5/S7 would give artificial MCC=1 (the stale-BSM
    //     injection creates a detectable jump even with no real RSU infrastructure).
    //     Use RSU-level counters only → correctly gives MCC=0 (detector inoperable).
    ...
    //   This is the correct convention: a detector that makes zero errors on a clean
    //   dataset has MCC=1, not MCC=0/0. The 0/0 formula gives the wrong result here.
    ...
    double acr   = (total > 0.0) ? ((tp + tn) / total * 100.0) :
