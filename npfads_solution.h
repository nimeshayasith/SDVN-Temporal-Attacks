...
public:
    ...
    void SetAttackScenario(int s)       { m_attackScenario = s; }
...
private:
    ...
    void RunModeB_RFSimulated()
    {
        ...
        for (size_t i = 0; i < m_eigRecs.size(); ++i) {
            ...
            // R4: slightly-below-normal variance → sub-benign motion (possible freeze)
            // Catches 0.10 ≤ ratio < 0.25: below benign mean but not as extreme as R1.
            // Uses only posVar ratio — no ground-truth label.
            if (ratio < 0.25) {
                sr.rfLabel = 1;
                sr.rfAlert = true;
                continue;
    ...
    void RunModeC_NADM()
    {
        ...
        //
        // S(X) is approximated from eigenvalue features only — no ground-truth
        // label is accessed.  Known-class centroids in posVar-ratio space:
        //   Type 1 centroid: ratio ≈ 0  (near-zero position variance)
        //   Type 2 centroid: ratio ≈ 1  (constant offset → removed by centering)
        // A sample close to either centroid gets high S(X); far from both → low S(X).
        // This replaces the previous switch(er.attackType) which read the label
        // before producing the detection output — a ground-truth leakage bug.
        //
        // Note: τ_1 / τ_2 are paper Table-3 (H3) values used as fixed thresholds.
        // In a fully trained system these would be derived from CCR/MCR sweeps
        // on the training split (see fix_plan.md Phase C, steps C8–C9).
        static const double TAU_1 = 0.989;  // H3 threshold for predicted Type 1
        static const double TAU_2 = 0.999;  // H3 threshold for predicted Type 2

        // S(X): RF prediction confidence estimated from eigenvalue features only.
        auto SimulateSx = [&](const NpfadsEigRecord& er) -> double {
            double ratio    = er.posVar / m_benignMeanPosVar;
            double d_type1  = ratio;                  // distance from Type 1 centroid (ratio=0)
            double d_type2  = std::abs(ratio - 1.0);  // distance from Type 2 centroid (ratio=1)
            double min_dist = std::min(d_type1, d_type2);
            // Closer to a known-class centroid → higher confidence → higher S(X).
            double sx = 1.0 / (1.0 + min_dist);
            return std::max(0.30, std::min(0.999, sx));
        };
        ...
        for (size_t i = 0; i < m_eigRecs.size(); ++i) {
            ...
            // Step 1: predict the known class from eigenvalue features (no label).
            if (er.posVar < m_benignMeanPosVar * 0.5)
                sr.nadmRfLabel = NPFADS_TYPE_1;  // near-zero posVar → Type 1
            else if (er.posVar > m_benignMeanPosVar * 5.0)
                sr.nadmRfLabel = NPFADS_TYPE_1;  // huge posVar → still closest to Type 1
            else
                sr.nadmRfLabel = NPFADS_TYPE_2;  // moderate → closest to Type 2

            // Step 2: select H3 threshold for the predicted class (no label).
            double tau_i   = (sr.nadmRfLabel == NPFADS_TYPE_2) ? TAU_2 : TAU_1;
    ...
    void ComputeMetrics()
    {
        ...
        for (int atype : attackTypes) {
            ...
            // ── Mode A: posVar threshold sweep ────────────────────────────────
            {
                ...
                double mcc = 0.0;   // undefined (no observations) → 0 = no detection capability
                {
                                            ...
                                            const std::string& filePrefix) const
{
    ...
    // Always truncate so each individual run produces a clean single-run file.
    // Accumulation across runs (for the plotting CSV) is handled by the shell
    // script (run_npfads_only.sh) which appends rows using tail -n +2.
    {
        std::string summaryPath = outputDir + "/" + filePrefix + "NPFADS_PEM_Run_Summary.csv";
        std::ofstream f(summaryPath, std::ios::out | std::ios::trunc);
        f << std::fixed << std::setprecision(6);
        f << "attack_percentage,attack_scenario,"
