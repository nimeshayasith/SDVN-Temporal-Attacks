...
# Strip attack_type=-1 (combined-all) rows from master KNN CSVs.
# These rows are generated by temporal_veremi_compare_knn.py for the
# "ALL SCENARIOS COMBINED" evaluation and have no scenario-specific meaning.
if [ -f "$MASTER_KNN_HOLDOUT" ]; then
    sed -i '/^-1,/d' "$MASTER_KNN_HOLDOUT"
fi
if [ -f "$MASTER_KNN_CV" ]; then
    sed -i '/^-1,/d' "$MASTER_KNN_CV"
fi
