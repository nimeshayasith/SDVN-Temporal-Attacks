...
- **Distinct from `g_rcomm` / `TTW_COMM_RANGE = 300.0` m** — that is the protocol/design communication-range parameter used for TTW's own placement/HELLO-geometry/link-break logic and BSHH-S3's `r_overlap` fallback, and must **not** change. `kEffectiveReceptionRadius` is only used for (a) scenario-construction vehicle selection (`MeSelectMutualRangePairNearRsu`, `TtwFindNaturalBreakPairs`, etc.) and (b) the ambient reception gates (`PemEmitVehicleBeacon`, `PemEmitNeighborObservation`), so attack scenarios are only built from vehicle placements that are physically realizable under the real Cost231 propagation + WifiPhy error-rate model.
- **UPDATE (2026-07-27, supervisor's detection-equation patch)**: a new `g_me_detect_range = 170.0` m constant now supersedes `TTW_COMM_RANGE` specifically for the *detection-equation-only* comparisons — ME-S1's ρ_max (Eq. 3.8, §4 below), the δ_thresh formula (Eq. 3.x), and ME's own link-reality checks (`link12`/`link34`/`srcDstLinked`/`v3v4_linked`). This does **not** change `TTW_COMM_RANGE` itself (still 300m, still governs TTW's placement/HELLO/link-break logic and BSHH-S3's `r_overlap`), and does not change `kEffectiveReceptionRadius` (still 100m, still the real achievable DSRC range) — three genuinely distinct constants for three genuinely distinct purposes, not one value with multiple names. See `TGN_HYPERPARAMETER_CALIBRATION.md` §3g for why this required a TGN dataset regen + retrain (it feeds `TGN_RecalibrateMobility()`'s live `W_max`) while the other 170m changes did not.
...
- **Confirmed root cause of "ME-S1/S3/S4 undetected at N=200"**: `sig[6]` (ME-S1, Eq. 3.8) uses a density-adaptive threshold `rhoMax = floor((1+0.20) × 2 × r_comm × λ̂(t))` (`PemComputeRhoMaxForLink`, `routing.cc:4884`) — NOT a fixed threshold. At N=200, ambient vehicle density `λ̂(t)` is high enough that `rhoMax` comfortably exceeds a ~20-phantom-reporter attack (an implicit α_atk ≈ 0.10, not the paper's 0.20). This is **Eq. 3.8 working as designed**, not a bug — confirmed by re-running at small scale (N=20, `--test_network=1`): same code, same formula, reporter_count still climbs correctly, and 9/10 attack rows get flagged once the attacker-to-density ratio is realistic.
  - **UPDATE (2026-07-27)**: `r_comm` in this specific formula now means `g_me_detect_range = 170.0` m, not `TTW_COMM_RANGE = 300.0` m — see §1's update note above. `rhoMax` is therefore smaller than it would have been under the old 300m value (the formula scales linearly with `r_comm`), making detection *more* sensitive at a given density, not less — this doesn't reopen the "undetected at N=200" finding, it should make it easier to clear the threshold, not harder.
...
- **PEM_ME_TOLERANCE_MU = 0.20** — the `µ` margin in Eq. 3.8's `rhoMax` formula (`(1+µ)·2·r_comm·λ̂(t)`), where `r_comm = g_me_detect_range = 170.0` m as of the 2026-07-27 patch (was `TTW_COMM_RANGE = 300.0` m before).
