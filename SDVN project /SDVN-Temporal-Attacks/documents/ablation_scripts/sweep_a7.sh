...
# r_comm, X in {1.0, 1.5, 2.0, 2.5, 3.0} -- 5 equal 0.5r steps, updated
# 2026-08-04, was 3 unequal points {1.0,1.5,3.0}). PDF scopes this to the
# ME defence specifically -- now covers all 4 ME variants (9=ME-S1,
# 10=ME-S2, 11=ME-S3, 12=ME-S4), matching A6's existing all-4-BSHH-variant
# pattern, instead of ME-S1 only. The --echo_dist_ratio reorder hook
# (MeReorderEchoCandidatesByDistanceRatio) is now wired into all 4 ME
# scheduling blocks, not just ME-S1 -- confirmed firing via smoke test on
# sc10/11/12 before this sweep was expanded. Design note: without
# location-binding, ME echo reports pass Stage 0 regardless of distance --
# this sweep shows at what reporter distance the LW ME-S1/S2/S3 signatures
# alone compensate.
set -e
...
for SC in 9 10 11 12; do
  NRSU=${RSU_FOR_SC[$SC]}
  for X in 1.0 1.5 2.0 2.5 3.0; do
