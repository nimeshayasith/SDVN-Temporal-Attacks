    ...
    newSigner(hsmSignerOptions) {
        ...
        return {
            signer: async (digest) => {
                ...
                const compactSignature = await pkcs11.C_SignAsync(session, Buffer.from(digest), 
                ...
                Buffer.alloc(nist_js_1.p256.Point.Fn.BYTES * 2));
                let signature = nist_js_1.p256.Signature.fromBytes(compactSignature, 'compact');
                if (signature.hasHighS()) {
                    const lowS = nist_js_1.p256.Point.Fn.neg(signature.s);
                    signature = new nist_js_1.p256.Signature(signature.r, lowS);
                }
