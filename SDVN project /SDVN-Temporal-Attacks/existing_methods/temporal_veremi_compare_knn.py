...
TYPE_NAMES = {
    ...
    # VeReMi dataset attack types (Mekonen 2025 original dataset)
    101: "VeReMi Type 1  — Position Frozen (EventualStop)",
    102: "VeReMi Type 2  — Random Position",
    104: "VeReMi Type 4  — Random Offset",
    108: "VeReMi Type 8  — Eventual Stop",
    116: "VeReMi Type 16 — Data Replay",
}
...
def evaluate_holdout(df_combined, attack_type, classifiers, test_split):
    ...
    for clf_name, clf in classifiers.items():
        ...
        # Vacuous-perfect case: no attack events in test set (all-negative ground truth)
        # but baseline events exist → detector is trivially correct → MCC=1, AUROC=1.
        # This occurs for S2/S6 at attack_pct=0 (no attack injected, RSU traffic present).
        n_pos_truth = int((y_test == 1).sum())
        n_neg_truth = int((y_test == 0).sum())
        if n_pos_truth == 0 and n_neg_truth > 0:
            mcc   = 1.0
            auroc = 1.0
        else:
            # MCC = 0 when TP=0 (no attack detected) — expected for Temporal-Echo attacks.
            mcc = float(matthews_corrcoef(y_test, y_pred))
            # AUROC from predict_proba — falls back to 0.500 if only one class present.
            try:
...
# ─────────────────────────────────────────────────────────────
def load_veremi_dataset(dataset_dir):
    """
    Load the False_Position_Attack_Detection_Data dataset used by Mekonen et al. 2025.

    Files: at1.csv, at2.csv, at4.csv, at8.csv, at16.csv
    Each file columns:
      sendtime_1, sender_1, messageID,
      pos-x1, pos-y1, pos-z1, spd-x1, spd-y1, spd-z1,
      AttackerType,
      sendtime_2, sender_2,
      pos-x2, pos-y2, pos-z2, spd-x2, spd-y2, spd-z2

    Mapped to script format:
      sendtime_1          → sim_time_s
      sender_1            → vehicle_id
      pos-x1/y1           → pos_x1, pos_y1
      spd-x1/y1           → spd_x1, spd_y1
      pos-x2/y2           → pos_x2, pos_y2
      spd-x2/y2           → spd_x2, spd_y2
      sendtime_2-sendtime_1 → time_interval
      (AttackerType != 0) → label  (binary)
      file attack number  → attack_type (100+n to avoid collision with scenario IDs 1-12)

    Z-axis components (pos-z, spd-z) are dropped — planar 2D motion only.
    """
    import os

    file_map = {"at1.csv": 101, "at2.csv": 102, "at4.csv": 104,
                "at8.csv": 108, "at16.csv": 116}

    frames = []
    for fname, atype in file_map.items():
        fpath = os.path.join(dataset_dir, fname)
        if not os.path.exists(fpath):
            print(f"  [WARNING] {fname} not found in {dataset_dir} — skipped.")
            continue

        raw = pd.read_csv(fpath)
        df_f = pd.DataFrame()
        df_f["sim_time_s"]    = raw["sendtime_1"]
        df_f["vehicle_id"]    = raw["sender_1"].astype(int)
        df_f["pos_x1"]        = raw["pos-x1"]
        df_f["pos_y1"]        = raw["pos-y1"]
        df_f["spd_x1"]        = raw["spd-x1"]
        df_f["spd_y1"]        = raw["spd-y1"]
        df_f["pos_x2"]        = raw["pos-x2"]
        df_f["pos_y2"]        = raw["pos-y2"]
        df_f["spd_x2"]        = raw["spd-x2"]
        df_f["spd_y2"]        = raw["spd-y2"]
        df_f["time_interval"] = (raw["sendtime_2"] - raw["sendtime_1"]).clip(lower=1e-6)
        df_f["label"]         = (raw["AttackerType"] != 0).astype(int)
        df_f["attack_type"]   = atype  # 101/102/104/108/116

        n_legit  = int((df_f["label"] == 0).sum())
        n_attack = int((df_f["label"] == 1).sum())
        print(f"  Loaded {fname}: {len(df_f)} rows  "
              f"(legit={n_legit}, attack={n_attack}, type={atype})")
        frames.append(df_f)

    if not frames:
        raise FileNotFoundError(f"No VeReMi dataset files found in: {dataset_dir}")

    df_all = pd.concat(frames, ignore_index=True)
    print(f"  Combined: {len(df_all)} rows total  "
          f"(legit={int((df_all['label']==0).sum())}, "
          f"attack={int((df_all['label']==1).sum())})")
    return df_all


# ─────────────────────────────────────────────────────────────
def save_trained_model(classifiers, path):
    """Save all 4 trained classifier pipelines to a pickle file via joblib."""
    try:
        import joblib
        joblib.dump(classifiers, path)
        print(f"  Classifiers saved → {path}")
    except Exception as e:
        print(f"  [WARN] Could not save model: {e}")


def load_trained_model(path):
    """Load pre-trained classifiers dict from a joblib pickle file."""
    try:
        import joblib
        clfs = joblib.load(path)
        print(f"  Classifiers loaded from: {path}")
        return clfs
    except Exception as e:
...
def run_sim_test_mode(classifiers, test_csv_path, scenario, attack_pct, output_summary):
    """
    Test pre-trained classifiers on one NS-3 simulation pairs CSV.
    Appends a single row to output_summary (same format as comparison_veremi_summary.csv).
    Uses RF+Bagging as the representative classifier.
    """
    import os, math
    ...
    try:
        ...
        print(f"  ERROR loading test CSV {test_csv_path}: {e}")
        return

    if df_test.empty:
        print(f"  [KNN] Test CSV is empty — writing zero row.")
        attack_pct_f = float(attack_pct)
        pdr_attack = 100.0 * math.pow(1.0 - attack_pct_f / 100.0, 0.7) if attack_pct_f < 100 else 0.0
        # Empty CSV means denom=0 (no pairs) → MCC=0, AUROC=0.5.
        e_mcc, e_auroc = 0.0, 0.5
        write_header = (not os.path.exists(output_summary) or
                        os.path.getsize(output_summary) == 0)
        with open(output_summary, "a") as f:
            if write_header:
                f.write("attack_scenario,attack_percentage,tp,tn,fp,fn,"
                        "mcc,auroc,tdet_ms,pdr_attack\n")
            f.write(f"{scenario},{attack_pct},0,0,0,0,"
                    f"{e_mcc:.4f},{e_auroc:.4f},-1.0,{pdr_attack:.2f}\n")
        return

    # Rename to match internal column names
    df_test = df_test.rename(columns={"sim_time": "sim_time_s", "dt": "time_interval"})
    df_test = engineer_features(df_test)

    missing = [c for c in FEATURE_COLS if c not in df_test.columns]
    if missing:
        print(f"  ERROR: Missing features in test CSV: {missing}")
        return

    if "label" not in df_test.columns:
        print("  ERROR: No 'label' column in test CSV.")
        return

    X_test = df_test[FEATURE_COLS].values
    y_test = df_test["label"].values

    # RF+Bagging is the best performer — use as representative
    clf = classifiers.get("RF+Bagging") or next(iter(classifiers.values()))
    y_pred = clf.predict(X_test)
    ...
    # denom=0 cases (no positive predictions possible — degenerate):
    #   S1/S3/S5/S7/S9/S11: all zeros → denom=0 → MCC=0
    #   S4/S8/S12: TP=FP=0, TN/FN>0 → denom=0 → MCC=0
    # denom>0 cases:
    #   S10: TP=0 but FP>0,FN>0 possible → MCC<0 naturally (correct)
    #   S2/S6: TP>0 → MCC>0 (detectable)
    ...
    # AUROC — single-point estimate; 0.5 when no attack events (tp+fn=0).
    if (tp + fn) == 0:
        auroc = 0.5
    else:
        tpr   = tp / (tp + fn)
    ...
    # Tdet — time from first attack event to first TP detection (ms).
    # -1.0 means no attack events (0% attack) or no detection (TP=0).
    # Matches comparison_detector.h: tdet = (first_alert_time - attack_start) * 1000.
    tdet_ms = -1.0
    if "sim_time_s" in df_test.columns:
        attack_mask = (y_test == 1)
        if attack_mask.any():
            t_start = float(df_test["sim_time_s"].values[attack_mask].min())
            tp_mask = (y_test == 1) & (y_pred == 1)
            if tp_mask.any():
                tdet_ms = (float(df_test["sim_time_s"].values[tp_mask].min())
                           - t_start) * 1000.0
    ...
    with open(output_summary, "a") as f:
        if write_header:
            f.write("attack_scenario,attack_percentage,tp,tn,fp,fn,"
                    "mcc,auroc,tdet_ms,pdr_attack\n")
        f.write(f"{scenario},{attack_pct},{tp},{tn},{fp},{fn},"
                f"{mcc:.4f},{auroc:.4f},{tdet_ms:.1f},{pdr_attack:.2f}\n")

    print(f"  [CD-KNN] S{scenario} ATK={attack_pct}%: "
          f"TP={tp} FP={fp} FN={fn} TN={tn} "
          f"MCC={mcc:.4f} AUROC={auroc:.4f} Tdet={tdet_ms:.1f}ms "
          f"PDR={pdr_attack:.1f}%  (N={len(y_test)})")
    print(f"  → Appended to: {output_summary}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to False_Position_Attack_Detection_Data directory "
             "(Mekonen 2025 original dataset: at1.csv, at2.csv, at4.csv, at8.csv, at16.csv). "
             "When given, this takes priority over csv_file."
    )
    parser.add_argument(
        "--save-model", type=str, default=None,
        help="Train on --dataset and save all 4 classifiers to this pickle file (pre-train once)."
    )
    parser.add_argument(
        "--load-model", type=str, default=None,
        help="Load pre-trained classifiers from pickle file (use with --test-csv for fast inference)."
    )
    parser.add_argument(
        "--test-csv", type=str, default=None,
        help="Simulation pairs CSV to test on. Train source: --load-model or --dataset. "
             "Writes one summary row to --output-summary."
    )
    parser.add_argument(
        "--scenario", type=int, default=0,
        help="Attack scenario ID written to summary CSV row (default: 0)."
    )
    parser.add_argument(
        "--attack-pct", type=int, default=0,
        help="Attack percentage written to summary CSV row (default: 0)."
    )
    parser.add_argument(
        "--output-summary", type=str, default="comparison_knn_summary.csv",
        help="Output summary CSV path for per-run results (default: comparison_knn_summary.csv)."
    )
    args = parser.parse_args()

    # ── Save-model mode: train once on VeReMi, pickle for reuse ─────────────
    if args.save_model is not None:
        if args.dataset is None:
            print("ERROR: --save-model requires --dataset.")
            sys.exit(1)
        print(f"\nLoading VeReMi dataset from: {args.dataset}")
        try:
            df_tr = load_veremi_dataset(args.dataset)
        except Exception as e:
            print(f"ERROR: {e}")
            sys.exit(1)
        df_tr = engineer_features(df_tr)
        df_tr_legit  = df_tr[df_tr["label"] == 0]
        df_tr_attack = df_tr[df_tr["label"] == 1]
        df_balanced  = balance_dataset(df_tr_legit, df_tr_attack)
        if df_balanced is None:
            print("ERROR: Could not balance VeReMi training data.")
            sys.exit(1)
        X_tr = df_balanced[FEATURE_COLS].values
        y_tr = df_balanced["label"].values
        clfs = build_classifiers(args.n_estimators, args.n_neighbors)
        print(f"\nTraining {len(CLF_ORDER)} classifiers on {len(X_tr)} balanced pairs...")
        for clf_name, clf in clfs.items():
            print(f"  {clf_name} ...", end="  ", flush=True)
            clf.fit(X_tr, y_tr)
            print("done")
        save_trained_model(clfs, args.save_model)
        if args.test_csv is None:
            return
        # Fall through to test mode below using already-trained clfs
        classifiers_pretrained = clfs
    else:
        classifiers_pretrained = None

    # ── Test-csv mode: run inference on simulation pairs, write summary row ──
    if args.test_csv is not None:
        if classifiers_pretrained is not None:
            clfs_for_test = classifiers_pretrained
        elif args.load_model is not None:
            clfs_for_test = load_trained_model(args.load_model)
            if clfs_for_test is None:
                sys.exit(1)
            ...
            X_tr = df_balanced[FEATURE_COLS].values
            y_tr = df_balanced["label"].values
            clfs_for_test = build_classifiers(args.n_estimators, args.n_neighbors)
            print(f"\nTraining on {len(X_tr)} balanced VeReMi pairs...")
            for clf_name, clf in clfs_for_test.items():
                print(f"  {clf_name} ...", end="  ", flush=True)
                clf.fit(X_tr, y_tr)
                print("done")
        else:
            print("ERROR: --test-csv requires --load-model or --dataset.")
            sys.exit(1)
        ...
        return   # done — skip full evaluation
    ...
    if args.dataset is not None:
        # Load from the original Mekonen 2025 VeReMi dataset directory
        print(f"\nLoading VeReMi dataset from: {args.dataset}")
        try:
            df = load_veremi_dataset(args.dataset)
        except Exception as e:
            print(f"ERROR loading VeReMi dataset: {e}")
            sys.exit(1)
        data_source = args.dataset
    else:
        # Load from simulation-generated pairs CSV (default)
        try:
