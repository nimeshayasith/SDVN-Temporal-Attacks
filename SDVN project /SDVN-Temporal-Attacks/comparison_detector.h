...
static void CD_BsmTick(double t_end)
{
    ...
    double now    = ns3::Simulator::Now().GetSeconds();
    bool   attack = (cd_pem_attack_active != nullptr) && (*cd_pem_attack_active);
...
static void CD_CapturePositions()
{
    ...
    bool is_rsu_attacker = (cd_attack_scenario == 2 || cd_attack_scenario == 6);
                            ...
                            uint32_t n_rsus)
{
    ...
    //
    // Structural cases that arise across the 12 scenarios:
    //   S1,S3,S5,S7,S9,S11 (no RSU): no pairs → TP=TN=FP=FN=0 → denom=0 → MCC=0
    //   S4,S8,S12 (mal.ctrl + RSU): TP=0,FP=0,TN>0,FN>0 → denom=0 → MCC=0
    //   S10 (ME mal.RSU): TP=0 but FP>0,FN>0 possible → denom>0 → MCC<0 (correct)
    //   S2,S6 (mal.RSU forged BSMs): TP>0 → denom>0 → MCC positive (detectable)
    //
    // When the denominator is zero the formula is undefined; output MCC=0
    // (the classifier makes no positive predictions — no detection signal).
    ...
    // When tp+fn=0 there are no attack events to evaluate → 0.5 (undefined/random).
    if (cd_tp + cd_fn == 0) {
        auroc = 0.5;
    } else {
        double tpr = tp / (tp + fn);
